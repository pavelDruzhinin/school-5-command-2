<template>
  <b-modal title="Добавление вопроса" @ok="savequestion()" id="modal" scrollable>
    ...
    <b-form-group v-if="question.type" label="Варианты ответов:" label-for="variants">
      <b-button @click="addvariant()" variant="primary">Добавить вариант</b-button>
      <b-list-group id="variants">
        <draggable v-model="question.buttons">
          <b-list-group-item class="my-2" v-for="(button,index) in question.buttons" :key="index">
            <b-form-textarea v-model="button.text" />
            <b-button class="mt-2" @click="deletevariant(index)">Удалить</b-button>
          </b-list-group-item>
        </draggable>
      </b-list-group>
    </b-form-group>
